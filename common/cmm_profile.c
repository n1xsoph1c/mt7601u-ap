		...
		IF_DEV_CONFIG_OPMODE_ON_AP(pAd)
		{
			for (i = 0, macptr = rstrtok(tmpbuf,";"); macptr; macptr = rstrtok(NULL,";"), i++)
			{
				...
				if((KeyIdx >= 1 ) && (KeyIdx <= 4))
					pAd->ApCfg.MBSSID[i].DefaultKeyId = (unsigned char) (KeyIdx - 1 );
				else
	...
	IN char *buffer)
{
	...
	if(RTMPGetKeyParameter("ApCliSsid", tmpbuf, MAX_PARAM_BUFFER_SIZE, buffer, FALSE))
	{
		for (i = 0, macptr = rstrtok(tmpbuf,";"); (macptr && i < MAX_APCLI_NUM); macptr = rstrtok(NULL,";"), i++) 
		{
			...
			pApCliEntry->CfgSsidLen = (unsigned char)strlen(macptr);
	...
	if (RTMPGetKeyParameter("ApCliDefaultKeyID", tmpbuf, 255, buffer, TRUE))
	{
		for (i = 0, macptr = rstrtok(tmpbuf,";"); (macptr && i < MAX_APCLI_NUM); macptr = rstrtok(NULL,";"), i++)
		{
			...
			KeyIdx = simple_strtol(macptr, 0, 10);
			if((KeyIdx >= 1 ) && (KeyIdx <= 4))
				pApCliEntry->DefaultKeyId = (unsigned char) (KeyIdx - 1);
			else
	...
	if(RTMPGetKeyParameter("ApCliAPSDCapable", tmpbuf, 10, buffer, TRUE))
	{
			...
			macptr = rstrtok(NULL,";"), i++)
		{
			...
			pApCliEntry->UapsdInfo.bAPSDCapable = (unsigned char) simple_strtol(macptr, 0, 10);
...
static void rtmp_read_ap_wmm_parms_from_file(IN  PRTMP_ADAPTER pAd, char *tmpbuf, char *buffer)
{
	...
	if(RTMPGetKeyParameter("WmmCapable", tmpbuf, 32, buffer, TRUE))
	{
		...
		for (i = 0, macptr = rstrtok(tmpbuf,";"); macptr; macptr = rstrtok(NULL,";"), i++)
		{
			...
			pAd->ApCfg.MBSSID[i].bWmmCapableOrg = pAd->ApCfg.MBSSID[i].bWmmCapable;
	...
	if(RTMPGetKeyParameter("APAifsn", tmpbuf, 32, buffer, TRUE))
	{
		for (i = 0, macptr = rstrtok(tmpbuf,";"); macptr; macptr = rstrtok(NULL,";"), i++)
		{
			pAd->CommonCfg.APEdcaParm.Aifsn[i] = (unsigned char) simple_strtol(macptr, 0, 10);;
	...
	if(RTMPGetKeyParameter("APCwmin", tmpbuf, 32, buffer, TRUE))
	{
		for (i = 0, macptr = rstrtok(tmpbuf,";"); macptr; macptr = rstrtok(NULL,";"), i++)
		{
			pAd->CommonCfg.APEdcaParm.Cwmin[i] = (unsigned char) simple_strtol(macptr, 0, 10);;
	...
	if(RTMPGetKeyParameter("APCwmax", tmpbuf, 32, buffer, TRUE))
	{
		for (i = 0, macptr = rstrtok(tmpbuf,";"); macptr; macptr = rstrtok(NULL,";"), i++)
		{
			pAd->CommonCfg.APEdcaParm.Cwmax[i] = (unsigned char) simple_strtol(macptr, 0, 10);;
	...
	if(RTMPGetKeyParameter("BSSAifsn", tmpbuf, 32, buffer, TRUE))
	{
		for (i = 0, macptr = rstrtok(tmpbuf,";"); macptr; macptr = rstrtok(NULL,";"), i++)
		{
			pAd->ApCfg.BssEdcaParm.Aifsn[i] = (unsigned char) simple_strtol(macptr, 0, 10);;
	...
	if(RTMPGetKeyParameter("BSSCwmin", tmpbuf, 32, buffer, TRUE))
	{
		for (i = 0, macptr = rstrtok(tmpbuf,";"); macptr; macptr = rstrtok(NULL,";"), i++)
		{
			pAd->ApCfg.BssEdcaParm.Cwmin[i] = (unsigned char) simple_strtol(macptr, 0, 10);;
	...
	if(RTMPGetKeyParameter("BSSCwmax", tmpbuf, 32, buffer, TRUE))
	{
		for (i = 0, macptr = rstrtok(tmpbuf,";"); macptr; macptr = rstrtok(NULL,";"), i++)
		{
			pAd->ApCfg.BssEdcaParm.Cwmax[i] = (unsigned char) simple_strtol(macptr, 0, 10);;
	...
	if(RTMPGetKeyParameter("AckPolicy", tmpbuf, 32, buffer, TRUE))
	{
		for (i = 0, macptr = rstrtok(tmpbuf,";"); macptr; macptr = rstrtok(NULL,";"), i++)
		{
			pAd->CommonCfg.AckPolicy[i] = (unsigned char) simple_strtol(macptr, 0, 10);;
	...
	if(RTMPGetKeyParameter("APSDCapable", tmpbuf, 10, buffer, TRUE))
	{
		...
		for (i = 0, macptr = rstrtok(tmpbuf,";"); macptr; macptr = rstrtok(NULL,";"), i++)
		{
			...
			{
				pAd->ApCfg.MBSSID[i].UapsdInfo.bAPSDCapable = (unsigned char) simple_strtol(macptr, 0, 10);
	...
	while (RTMPGetKeyParameterWithOffset("RADIUS_Key", tmpbuf, &offset, 640, buffer, FALSE))
	{
		...
		for (i=0, macptr = rstrtok(tmpbuf,";"); (macptr && i < MAX_MBSSID_NUM(pAd)); macptr = rstrtok(NULL,";"), i++) 
		{
			if (strlen(macptr) > 0 && (count[i] < pAd->ApCfg.MBSSID[i].radius_srv_num))
			{
				...
				count[i] ++;
				DBGPRINT(RT_DEBUG_TRACE, ("IF(ra%d), radius_key(seq-%d) = %s, len = %d\n", i, count[i], pAd->ApCfg.MBSSID[i].radius_srv_info[srv_idx].radius_key, pAd->ApCfg.MBSSID[i].radius_srv_info[srv_idx].radius_key_len));
			}
		}
	}
	...
	for (i = 0; i < pAd->ApCfg.BssidNum; i++)
	{
		...
		if (RTMPGetKeyParameter(tok_str, tmpbuf, 33, buffer, FALSE))
		{
			if (strlen(tmpbuf) > 0)
			{
				...
				DBGPRINT(RT_DEBUG_TRACE, ("IF-ra%d NAS-ID = %s, len = %d\n", i, pAd->ApCfg.MBSSID[i].NasId, pAd->ApCfg.MBSSID[i].NasIdLen));
			}
		}
	}
	...
	IN	char *pInput)
{
	...
	{
		Value = simple_strtol(pValueStr, 0, 10);
		pAd->CommonCfg.bHTProtect = ((Value == 1) ? TRUE : FALSE);
	...
	{
		Value = simple_strtol(pValueStr, 0, 10);
		pAd->CommonCfg.bBADecline = ((Value == 1) ? TRUE : FALSE);
	...
	{
		Value = simple_strtol(pValueStr, 0, 10);
		pAd->HTCEnable = ((Value == 1) ? TRUE : FALSE);
	...
	{
		Value = simple_strtol(pValueStr, 0, 10);
		pAd->CommonCfg.BACapability.field.AmsduEnable = ((Value == 1) ? TRUE : FALSE);
	...
	{
		Value = simple_strtol(pValueStr, 0, 10);
		pAd->CommonCfg.RegTransmitSetting.field.ShortGI = ((Value == GI_400) ? GI_400 : GI_800);
	...
	{
		Value = simple_strtol(pValueStr, 0, 10);
		pAd->CommonCfg.RegTransmitSetting.field.HTMODE = ((Value == HTMODE_GF) ? HTMODE_GF : HTMODE_MM);
	...
	if (RTMPGetKeyParameter("FixedTxMode", pValueStr, 25, pInput, TRUE))
	{
		...
		IF_DEV_CONFIG_OPMODE_ON_AP(pAd)
		{
			...
			{
				pAd->ApCfg.MBSSID[i].DesiredTransmitSetting.field.FixedTxMode = RT_CfgSetFixedTxPhyMode(Bufptr);
				DBGPRINT(RT_DEBUG_TRACE, ("(IF-ra%d) Fixed Tx Mode = %d\n", i, pAd->ApCfg.MBSSID[i].DesiredTransmitSetting.field.FixedTxMode));
			}
		}
#endif /* CONFIG_AP_SUPPORT */
	}
	...
	{
		Value = simple_strtol(pValueStr, 0, 10);
		pAd->CommonCfg.RegTransmitSetting.field.BW = ((Value == BW_40) ? BW_40 : BW_20);
	...
	{
		Value = simple_strtol(pValueStr, 0, 10);
		pAd->CommonCfg.RegTransmitSetting.field.EXTCHA = ((Value == 0) ? EXTCHA_BELOW : EXTCHA_ABOVE);
	...
	if (RTMPGetKeyParameter("HT_MCS", pValueStr, 50, pInput, TRUE))
	{
		...
		IF_DEV_CONFIG_OPMODE_ON_AP(pAd)
		{
			for (i = 0, Bufptr = rstrtok(pValueStr, ";"); (Bufptr && i < MAX_MBSSID_NUM(pAd)); Bufptr = rstrtok(NULL,";"), i++) 	
			{
				...
				DBGPRINT(RT_DEBUG_TRACE, ("(ra%d) HT: MCS = %s(%d)\n", i, 
						(pAd->ApCfg.MBSSID[i].DesiredTransmitSetting.field.MCS == MCS_AUTO ? "Automatic" : "Fixed"),
	...
	{
		Value = simple_strtol(pValueStr, 0, 10);
		pAd->CommonCfg.RegTransmitSetting.field.STBC = ((Value == STBC_USE) ? STBC_USE : STBC_NONE);
	...
	{
		Value = simple_strtol(pValueStr, 0, 10);
		pAd->CommonCfg.bForty_Mhz_Intolerant = ((Value == 1) ? TRUE : FALSE);
	...
	{
		Value = simple_strtol(pValueStr, 0, 10);
		pAd->ApCfg.bGreenAPEnable = ((Value == 1) ? TRUE : FALSE);
	...
	{
		Value = simple_strtol(pValueStr, 0, 10);
		pAd->CommonCfg.HT_DisallowTKIP = ((Value == 1) ? TRUE : FALSE);
...
/*	tmpbuf = kmalloc(MAX_PARAM_BUFFER_SIZE, MEM_ALLOC_FLAG);*/
	...
	do
	{
		...
		{
			pAd->bEEPROMFile = (unsigned char) simple_strtol(tmpbuf, 0, 10);
		...
		{
			unsigned char Geography = (unsigned char) simple_strtol(tmpbuf, 0, 10);
			if (Geography <= BOTH)
			{
		...
		IF_DEV_CONFIG_OPMODE_ON_AP(pAd)
		{
			...
			{
				pAd->ApCfg.BssidNum = (unsigned char) simple_strtol(tmpbuf, 0, 10);
		...
		{
			pAd->CommonCfg.Channel = (unsigned char) simple_strtol(tmpbuf, 0, 10);
	...
	/* DFSIndoor */
	{
		...
		}
		{
			...
			{
				pRadarDetect->SymRoundFromCfg = (unsigned char) simple_strtol(tmpbuf, 0, 10);
			...
			{
				pRadarDetect->BusyIdleFromCfg = (unsigned char) simple_strtol(tmpbuf, 0, 10);
			...
			{
				pRadarDetect->DFSParamFromConfig = (unsigned char) simple_strtol(tmpbuf, 0, 10);
		...
		IF_DEV_CONFIG_OPMODE_ON_AP(pAd)
		{
			...
			{
				pAd->ApCfg.DtimPeriod = (unsigned char) simple_strtol(tmpbuf, 0, 10);
...
/*#endif*/
		...
		*/
		...
		IF_DEV_CONFIG_OPMODE_ON_AP(pAd)
		{
			...
			{	
				unsigned char PhyMode = simple_strtol(tmpbuf, 0, 10);
				pAd->CommonCfg.MCastPhyMode.field.BW = pAd->CommonCfg.RegTransmitSetting.field.BW;
				switch (PhyMode)
				{
			...
			{
				unsigned char Mcs = simple_strtol(tmpbuf, 0, 10);
		...
		if (RTMPGetKeyParameter("WscV2Support", tmpbuf, 32, pBuffer, TRUE))
		{
			...
			IF_DEV_CONFIG_OPMODE_ON_AP(pAd)
			{
				for (i = 0, macptr = rstrtok(tmpbuf,";"); macptr; macptr = rstrtok(NULL,";"), i++)
				{
					if (i >= pAd->ApCfg.BssidNum)
						break;
					bEnable = (unsigned char)simple_strtol(macptr, 0, 10);
		...
		{
			unsigned char AntGain = simple_strtol(tmpbuf, 0, 10);
		...
		{
			unsigned char Bandedge = simple_strtol(tmpbuf, 0, 10);
...
#define LETTER_CASE_TRANSLATE(txt_p, card_id)			\
	...
	/* init*/
	os_alloc_mem(NULL, (unsigned char **)&buffer, MAX_INI_BUFFER_SIZE);
	if (buffer == NULL)
		return FALSE;
	...
	mac[0] = (unsigned char)(addr01 & 0xff);
	mac[1] = (unsigned char)(addr01 >> 8);
	mac[2] = (unsigned char)(addr23 & 0xff);
	mac[3] = (unsigned char)(addr23 >> 8);
	mac[4] = (unsigned char)(addr45 & 0xff);
	mac[5] = (unsigned char)(addr45 >> 8);
		...
		if (card_match_id >= 0) {
			...
			if (RTMPGetKeyParameter(card_id_buf, tmpbuf, 256, buffer, TRUE)) {
				if (strlen(tmpbuf) < sizeof(pAd->MC_FileName)) {
					...
					DBGPRINT(RT_DEBUG_TRACE, ("Card Profile Name = %s\n", pAd->MC_FileName));
				} else
					DBGPRINT(RT_DEBUG_ERROR, ("Card Profile Name length too large!\n"));
			} else
				DBGPRINT(RT_DEBUG_ERROR, ("Can not find search key word in card.dat!\n"));
	...
	IN char *buffer)
{
		...
		for (i = 0; i < MAX_WDS_ENTRY; i++) {
			...
			if (RTMPGetKeyParameter(tok_str, tmpbuf, 128, buffer, FALSE)) {
				if (pAd->MulTestTab.WdsEntry[i].WepStatus == Ndis802_11Encryption1Enabled) {
					...
					} /* Hex type */ else if (strlen(tmpbuf) == 10 || strlen(tmpbuf) == 26) {
						KeyLen = strlen(tmpbuf);
	...
	if(RTMPGetKeyParameter("WdsDefaultKeyID", tmpbuf, 10, buffer, TRUE)) {
		for (i = 0, macptr = rstrtok(tmpbuf,";"); (macptr && i < MAX_WDS_ENTRY); macptr = rstrtok(NULL, ";"), i++) {
			...
			if ((KeyIdx >= 1 ) && (KeyIdx <= 4))
				pAd->MulTestTab.WdsEntry[i].KeyIdx = (unsigned char) (KeyIdx - 1);
			else
		...
		for (readline = ptr = buffer, index=0; (ptr = strchr(readline, '\n')) != NULL; readline = ptr + 1, index++) {
			*ptr = '\0';
			...
			if (!strncmp(readline, "ch", 2)) {
				...
				os_alloc_mem(NULL, (unsigned char **)&pwr, sizeof(*pwr));
